...
class MusicService : MediaBrowserServiceCompat(), MusicPlayerCompletionListener, KoinComponent {
    ...
    private fun updateUI(song: Song?, isPlaying: Boolean?) {
        ...
        val remoteViews = MusicWidget.getRemoteViews(this)
        currFlipperState = if (song != null) {
            ...
            if (song.albumArt != null) {
                remoteViews.setImageViewBitmap(R.id.ivAlbumArt, song.albumArt)
            } else {
        ...
        val thisWidget = ComponentName(this, MusicWidget::class.java)
        ...
        if (song != null) {
            if (mNotification == null) {
                mNotification = MusicNotification(this, ONGOING_NOTIFICATION_ID,
                        song, isShuffleOn, mediaSession.sessionToken, channelId)
                startForeground(ONGOING_NOTIFICATION_ID, mNotification!!.notification)
            } else {
                val isPlayingUnboxed = isPlaying ?: false
                mNotification?.update(song, isPlayingUnboxed, isShuffleOn, mediaSession.sessionToken)
            }
        } else {
            stopForeground(true)
        }
